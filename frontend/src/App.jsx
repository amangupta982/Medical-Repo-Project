...
const PAGE_META = {
  '/': { title: 'National PHC Dashboard', sub: 'Real-time intelligence across the primary healthcare network' },
  '/map': { title: 'PHC Network Map', sub: 'Karnataka • 60 facilities across 10 districts' },
  '/stockout': { title: 'Stockout Risk Prediction', sub: '7-day early warning — LightGBM champion with SHAP explanations' },
  '/demand': { title: 'Demand Forecasting', sub: 'Multi-horizon medicine demand — 1d / 7d / 14d / 30d trained models' },
  '/emergency': { title: 'Emergency / What-If Simulation', sub: 'Stress-test against outbreak & supply-chain scenarios' },
  ...
  '/alerts': { title: 'System Alerts', sub: 'Prediction alerts and system notifications' },
}

export default function App() {
  const { theme, toggleTheme } = useTheme()
  const location = useLocation()
  const meta = PAGE_META[location.pathname] || { title: 'BRICS Health Platform', sub: '' }
  const [online, setOnline] = useState(true)
  const [now, setNow] = useState(new Date())

  useEffect(() => {
    const check = async () => {
      try { const r = await api.checkHealth(); setOnline(r?.status === 'healthy') }
      catch { setOnline(false) }
    }
    check()
    const iv = setInterval(() => { check(); setNow(new Date()) }, 8000)
  ...
  return (
    <div className={`flex min-h-screen ${isDark ? 'bg-[#0b1120]' : 'bg-slate-50'}`}>
      ...
      <div className="ml-[260px] flex-1 flex flex-col min-h-screen">
        ...
        `}>
          <div>
            ...
            <p className={`text-xs mt-0.5 ${isDark ? 'text-slate-500' : 'text-slate-400'}`}>{meta.sub}</p>
          </div>
